...
export default {
  ...
  computed: {
    ...
    placeholderInt() {
      const placeholder = this.field['x-attrs'] && this.field['x-attrs'].placeholder ? this.field['x-attrs'].placeholder;
      const fieldName = this.getFieldName(val);
      return placeholder || `${this.hasI18n ? this.getI18nTerm('form.select') : 'Select'} ${fieldName}`;
    },
    fieldType() {
