...
export default {
  ...
  methods: {
    ...
    getFieldName(val) {
      return val.title || this.hasI18nTerm(`form.${val.name}` || val.name);
    },
    getPlaceholder(val) {
      const placeholder = val['x-attrs'] && val['x-attrs'].placeholder
      return placeholder ? placeholder : `this.hasI18nTerm('form.select')` $t('form.select') + ' ' + getFieldName(element)
    },
  },
};
</script>
